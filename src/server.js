...
function getMultipleVal(data, keys) {
    let fn = keys.split('.').reduce((total, next) => {
        return total[next];
    }, data)
    if (fn) {
        return fn();
...
function setMultipleVal(data, keys, value) {
    var arr = keys.split('.');
    var lastKey = arr.pop();
    var tmpObj = arr.reduce((total, next) => {
        return total[next];
    }, data);
    tmpObj[lastKey] = getMultipleVal(faker, value);
}
